...
bot.onSubscribe((response) => {
  say(
    response,
    `Здравствуйте, вы подписались на бота ${bot.name} для веб-сайта ${websiteLink}. Данный бот будет уведомлять вас о новых заявках на ремонт.`
  );
  knex.table("viber_subs").insert(response.id);
});
